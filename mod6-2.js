...
G.props['new day lines']=[
  ...
  new G.Unit({
		...
		//upkeep:{'coin':0.2},
    modes:{
    'G A':{name:'G A',icon:[0,9],desc:'Get some [Autisum].'},
		'S to A':{name:'S to A',icon:0,desc:'Turn [spoiled food]s into [Autisum].',req:{'Spoiled Autisum':true}},
      },
		effects:[
			{type:'gather',what:{'insight':10}},
      {type:'convert',from:{'spoiled food':1},into:{'Autsium':1},every:5,mode:'S to A'},
		],
		req:{'speech':true},
		category:'discovery',
		priority:5,
  ...
  new G.Tech({
		...
		desc:'@[E]s can now produce [Austisum] from [spoiled food]s.',
		icon:0,
		cost:{'insight':20},
		req:{'symbolism:true},
	});
